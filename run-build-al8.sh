...
docker exec al8 dnf install -y wget bash openssl gcc gcc-c++ cmake m4 pkgconf clang llvm glibc-devel git
docker exec al8 dnf install --allowerasing -y coreutils binutils findutils util-linux sed gawk tar xz gzip bzip2 file
docker exec al8 /bin/bash -c 'ln -svf bash /bin/sh'
docker exec al8 dnf update -y
docker exec al8 /bin/bash -c 'rm -fr /tmp/*'
docker cp al8 al8:/home/
docker exec al8 /bin/bash /home/al8/brg.sh
